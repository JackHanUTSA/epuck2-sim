...
from obstacle_avoidance_logic import (
    AvoidanceState,
    MAX_SPEED,
...
def main():
    ...
    navigation_state = WaypointNavigatorState(waypoints=build_lawnmower_waypoints())
    ...
    required_sweeps = int(os.environ.get("EPUCK_REQUIRED_SWEEPS", "1"))
    dt = TIME_STEP / 1000.0
    map_frame_index = 0
    mapping_complete = False
    print(f"[e-puck2] Mapping output: {map_output_path}")
    print(f"[e-puck2] Exploration waypoints: {len(navigation_state.waypoints)} | Required sweeps: {required_sweeps}")
    if map_record_dir:
    ...
    while robot.step(TIME_STEP) != -1:
        ...
        # Compute wheel speeds. In clear space the robot tracks lawnmower
        # waypoints across the arena; when a wall or box is detected ahead,
        # it falls back to committed obstacle avoidance until the front clears.
        ...
        if step_count % (2000 // TIME_STEP) == 0:
            ...
            target_x, target_y = navigation_state.current_target()
            print(
                f"[e-puck2] Step {step_count} | "
                f"Front: {front_avg:.2f} | "
                f"L: {left_speed:.2f} R: {right_speed:.2f} | "
                f"Target: ({target_x:.2f}, {target_y:.2f}) | "
                f"Sweeps: {navigation_state.completed_sweeps}"
            )
        ...
        if save_period > 0 and step_count % save_period == 0:
            ...
            tracker_done = completion_tracker.update(mapping_state)
            if tracker_done and navigation_state.completed_sweeps >= required_sweeps:
                mapping_complete = True
                print(
                    f"[e-puck2] Mapping completion detected at step {step_count} "
                    f"after {navigation_state.completed_sweeps} sweep(s)"
                )
                if map_done_file:
